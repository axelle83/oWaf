...
class Novo_Map_Admin {
	...
	public function enqueue_post_admin_script() {
        wp_enqueue_script( $this->plugin_name.'-gmap-api', 'https://maps.googleapis.com/maps/api/js?v=3.31&key=' . esc_attr(get_option($this->plugin_name.'_gmap_api_key')) . '', array(), null, false );
	}
    ...
    public function post_admin_menu_callback() {
        ...
        // don't show marker menu if post not published
        if ($post_status == 'draft' || $post_status == 'auto-draft') {
            _e('<strong>The Post must be published to add a Marker</strong>', $this->plugin_name);
        }
        else {
            //add post menu html
            ob_start();
    ...
    public function save_post_admin_data($post_id) {
        ...
        if(! empty($novo_post) and $post['post_type'] != 'revision') {
            ...
            if(wp_is_post_revision($novo_post['novo-map-marker-post_id']) == false) {
                ...
                //add, update, delete marker
                if(isset($post[$this->plugin_name.'-marker-add'])) {
                    $marker = new Marker($novo_post);
                    $id = $manager->add($marker);
                }
                if(isset($post[$this->plugin_name.'-marker-update'])) {
                    $marker = new Marker($novo_post);
                    $manager->update($marker);
                }
                if(isset($post[$this->plugin_name.'-marker-delete'])) {
                    $marker = new Marker($novo_post);
        ...
        // --> only update tags and categories of marker if changed in post
